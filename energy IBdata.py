...
class NN:
    ...
    def train(self):#, nIter, E_star, Ne_star, Te_star, X_star):
        ...
        #         print('Error E: %e, Error Ne: %e, Error Te: %e' % (error_E, error_Ne, error_Te))
...
if __name__ == "__main__":
    ...
    X_u_train1 = X_u_train[idx_u, :]
    E_train1 = E_train[idx_u, :]
    Ne_train1 = Ne_train[idx_u, :]
    Te_train1 = Te_train[idx_u, :]
    idx_u = np.random.choice(X_star.shape[0], N, replace=False)
    X_u_train2 = X_star[idx_u, :]
    E_train2 = E_star[idx_u, :]
    Ne_train2 = Ne_star[idx_u, :]
    Te_train2 = Te_star[idx_u, :]

    X_u_train = np.vstack([X_u_train1, X_u_train2])
    E_train = np.vstack([E_train1, E_train2])
    Ne_train = np.vstack([Ne_train1, Ne_train2])
    Te_train = np.vstack([Te_train1, Te_train2])
    ...
    model.train()#10000, E_star, Ne_star, Te_star, X_star)
